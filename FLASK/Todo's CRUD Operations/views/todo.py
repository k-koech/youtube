...
from models import Tag, Todo, db
...
def update_todo(todo_id):
    ...
    if todo and todo.user_id==current_user_id:
        ...
        deadline_value = data.get('deadline', todo.deadline)
        if isinstance(deadline_value, datetime):
            deadline = deadline_value 
        else:
            deadline = datetime.strptime(deadline_value, '%Y-%m-%d %H:%M:%S') 

        check_tag_id =Tag.query.get(tag_id)
   
        if not check_tag_id :
            return jsonify({"error":"Tag/user doesn't exists"}),404
